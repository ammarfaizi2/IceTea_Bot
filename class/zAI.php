...
class AI
{
    ...
    public function __construct()
    {
        ...
        $this->sapa = array('dini hari','dini hari','dini hari','dini hari','pagi','pagi','pagi','pagi','pagi','menjelang siang','siang','siang','siang','siang','sore','sore','sore','sore','malam','malam','malam','malam','malam','malam','dini');
        $this->jadwal    = array(
"Senin"        =>"Senin\n\nBiologi\nKewirausahaan\nMatematika\nMatematika\nKimia\nKimia\nFisika\n\nPulang jam 16.00",
"Selasa"    =>"Selasa\n\nGeografi\nGeografi\nEkonomi\nEkonomi\nMatematika\nSeni Budaya\nAgama\nKeiran (Japan)\n\nPulang jam 16.30",
"Rabu"        =>"Rabu\n\nB.Indonesia\nB.Indonesia\nB.Inggris\nB.Inggris\nBiologi\nBiologi\nAgama\nMatematika\nMatematika\n\nPulang jam 15.30",
...
"bot"=>array(array(
    ...
    public function messages_check($haystack, $needle, $word_identical=false, $special_time=false, $max_length=null, $max_word=null, $time_exec=null, $word_exception=null)
    {
        if ($time_exec!==null) {
            if (!is_array($time_exec)) {
                throw new TimeException("Time execution range must be array !", 70);
            } elseif (!in_array((int)date("H"), $time_exec)) {
                return false;
            }
        }
        if ($word_exception!==null) {
            $word_exception = explode(",", $word_exception);
            foreach ($word_exception as $val) {
    ...
    private function action($key, $action="", $actor=null)
    {
        if ($key=="off"||$key=="on") {
            if (in_array($actor, $this->superuser)) {
                $key=="off" and file_put_contents("bot_off", 1) and $qa="bot_off" or unlink("bot_off") and $qa="bot_on" ;
                return array("text",$qa,null);
            }
            return array("text","apa itu?...",null);
        } elseif ($key=="ask") {
            $actor = explode(" ", $actor);
            $a = $this->ask($action);
        ...
        } elseif ($key=='jadwal'||$key=='jadwalku') {
            ...
            } elseif (strpos($action, 'kemarin')!==false) {
                $text = $this->jadwal[$this->hari[date('w', strtotime(date('Y-m-d')."-1day"))]];
            } elseif (strpos($action, '2 hari+')!==false||strpos($action, '2 hari kedepan')!==false) {
                $text = $this->jadwal[$this->hari[date('w', strtotime(date('Y-m-d')."+2day"))]];
            } elseif (strpos($action, '3 hari+')!==false||strpos($action, '3 hari kedepan')!==false) {
                $text = $this->jadwal[$this->hari[date('w', strtotime(date('Y-m-d')."+3day"))]];
            } elseif (strpos($action, '4 hari+')!==false||strpos($action, '4 hari kedepan')!==false) {
                $text = $this->jadwal[$this->hari[date('w', strtotime(date('Y-m-d')."+4day"))]];
            } elseif (strpos($action, '5 hari+')!==false||strpos($action, '5 hari kedepan')!==false) {
                $text = $this->jadwal[$this->hari[date('w', strtotime(date('Y-m-d')."+5day"))]];
            } elseif (strpos($action, '6 hari+')!==false||strpos($action, '6 hari kedepan')!==false) {
                $text = $this->jadwal[$this->hari[date('w', strtotime(date('Y-m-d')."+6day"))]];
            } elseif (in_array($action, $daylist)) {
        ...
        } elseif ($key=="shexec"||$key=="shell_exec") {
            if (in_array($actor, $this->superuser)) {
                $action = explode(" ", $this->msg_abs);
                unset($action[0]);
